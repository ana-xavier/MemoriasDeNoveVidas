...
func run() -> void:
	...
	character.path_finder_component.find_and_follow_path(Vector2(282, -24))
	await character.path_finder_component.arrived_at_target	
	character.path_finder_component.find_and_follow_path(Vector2(242, -24))
	...
	player.visible = true
	player.global_position = Vector2(12, 47)
